...
public class TokenProvider {
    ...
    public String createToken(Authentication authentication, Boolean rememberMe) {
        ...
        //MTC set header tenant Id based on user
        log.info("tenant is set to:(" + TenantContext.getCurrentTenant() +  ")   when creating token (should be default master db (sit))"  );
        log.info("creating JWT token with TENANT ID for user:" + authentication.getName());
        log.info("principal is:" + authentication.getPrincipal().toString());

        User prince = (User)authentication.getPrincipal();
        ...
        if (sitUser != null) {
            ...
            log.info("sitUser is:" + sitUser.getUsername() + "  companyDbname (TENANTID in token) is:" + sitUser.getCompany().getCompanyDbname()  );
            headerTenantId = sitUser.getCompany().getCompanyDbname();
        }else
        {
            log.info("could not find sitUser for username of: " + userName);
        }
        return Jwts.builder()
